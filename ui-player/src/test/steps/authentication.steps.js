...
When(
  "I confirm {string} as my password confirmation",
...
Then(
  "my account should be created with status {string}",
...
Then("my account should not be created", async function () {
  const account = await authService
    .getUser(this.testData.email)
    .catch(() => null);
...
Then(
  "my account should be created with the Google profile information",
  async function () {
...
Then(
  "my account should be created with the Apple profile information",
  async function () {
...
// Login steps
Given(
  "I have a verified account with email {string} and password {string}",
...
Given(
  "I have an unverified account with email {string}",
...
Then("I should see my display name in the header", async function () {
  await waitFor(() => {
    expect(
      screen.getByText(authService.getCurrentUser().displayName),
    ).toBeInTheDocument();
  });
});
