...
When(
  "I enter a different confirm password {string}",
...
Then(
  "I should see a message indicating verification code was sent to my email",
  function () {
    expect(
...
Then(
  "I should see a password strength indicator showing {string}",
  function (strength) {
...
Then(
  "I should see an error message about invalid verification code",
  async function () {
    await waitFor(() => {
      expect(
...
Then("I should see a message that a new code has been sent", async function () {
  await waitFor(() => {
    ...
    expect(
      screen.getByText(/code.*sent/i) || screen.getByText(/resent/i),
    ).toBeInTheDocument();
  });
});
...
Then("all password requirements should be marked as met", function () {
  const requirements = screen.getAllByText(
    /at least 8 characters|one uppercase|one lowercase|one number|one special/i,
  );
