...
describe('StorageService', () => {
  ...
  describe('constructor', () => {
    ...
    it('should initialize with custom base URL from environment', () => {
      expect(storageService.baseUrl).toBe('https://test-storage-api.com/storage');
    });
  });
  ...
  describe('uploadData', () => {
    ...
    it('should handle upload with minimal options', async () => {
      const mockFile = new File(['test'], 'minimal.txt', { type: 'text/plain' });
      const uploadOptions = {
    ...
    it('should throw error when upload fails', async () => {
      ...
      await expect(storageService.uploadData(uploadOptions)).rejects.toThrow('Upload failed: FormData creation failed');
  ...
  describe('remove', () => {
    it('should remove file successfully', async () => {
      ...
      // Should not throw
      await expect(storageService.remove(removeOptions)).resolves.toBeUndefined();
    });
    ...
    it('should throw error when remove fails', async () => {
      ...
      await expect(storageService.remove(removeOptions)).rejects.toThrow('Remove failed: Timeout failed');
  ...
  describe('getUrl', () => {
    ...
    it('should throw error when URL generation fails', async () => {
      ...
      await expect(storageService.getUrl(urlOptions)).rejects.toThrow('Get URL failed: URL construction failed');
  ...
  describe('exported functions', () => {
    it('should export uploadData function bound to singleton', async () => {
      const mockFile = new File(['test'], 'export-test.txt', { type: 'text/plain' });
  ...
  describe('edge cases', () => {
    ...
    it('should handle very long key names', async () => {
      ...
      expect(result.url).toBe(`https://test-storage-api.com/storage/${longKey}`);
    });
  });
});