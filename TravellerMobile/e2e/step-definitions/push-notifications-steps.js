...
Then(
  'I should receive a notification if the result is critical',
...
Then('I should see notification categories', async function (dataTable) {
  ...
  for (const [category, status] of Object.entries(categories)) {
    ...
    const toggle = element(
      by.id(`${category.toLowerCase().replace(' ', '-')}-toggle`),
    );
    if (status === 'ON') {
...
Given(
  'I receive multiple notifications while app is closed',
  async function (dataTable) {
...
When(
  'I force-touch \\(3D Touch) or long-press the notification',
  async function () {
...
Given('I have received multiple notifications', async function () {
  this.setTestData('notificationHistory', [
    'notification1',
    'notification2',
    'notification3',
  ]);
...
Then(
  'I should see options for different notification types',
  async function () {
...
When(
  'I set campaign invitations to use {string} sound',
  async function (soundName) {
...
When(
  'an urgent notification \\(session starting) would be sent',
  async function () {
    await device.sendUserNotification({
