...
Then(
  'I should see the progress indicator showing {string}',
...
Then('I should see {string} step', async function (stepName) {
  await DetoxHelper.waitForElement(
    `${stepName.toLowerCase().replace(' ', '-')}-step`,
  );
...
When(
  'I tap the {string} button next to {string}',
  async function (buttonType, skillName) {
    const skillElement = element(by.id(`skill-${skillName.toLowerCase()}`));
    await DetoxHelper.scrollUntilVisible(
      'skills-list',
...
When(
  'I long-press the {string} button next to {string}',
  async function (buttonType, skillName) {
...
Then(
  'I should see weapon options in a mobile-friendly grid',
  async function () {
...
Then(
  'I should see {string} in my character list',
