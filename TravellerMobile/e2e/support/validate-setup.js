...
class SetupValidator {
    ...
    try {
      ...
      if (fs.existsSync(detoxConfigPath)) {
        ...
        if (
          ...
          detoxConfig.configurations['ios.sim.debug']
        ) {
      ...
      if (this.checkFileExists(featureFile, `Feature file`)) {
        ...
        try {
          const content = fs.readFileSync(
            path.join(this.projectRoot, featureFile),
            'utf8',
          );
  ...
  checkDirectoryStructure() {
    ...
    const optionalDirs = ['e2e/reports', 'e2e/screenshots'];
  ...
  run() {
    ...
    if (this.errors.length === 0) {
      this.log(
        '🎉 Setup validation passed! All components are properly configured.',
      );
    } else {
      this.log(
        `❌ Setup validation failed with ${this.errors.length} errors:`,
        'error',
      );
