...
exports.seedCanonicalSystems = seedCanonicalSystems;
exports.seedTradeGoods = seedTradeGoods;
exports.seedTestUsers = seedTestUsers;
exports.seedTestCampaign = seedTestCampaign;
...
async function seedCanonicalSystems() {
    ...
    for (const systemData of CANONICAL_SYSTEMS) {
        ...
        for (const planetData of planets) {
            const uwp = (0, utilities_1.generateUWP)({
                starport: planetData.starport,
            ...
            const tradeCodes = (0, utilities_1.determineTradeClassifications)({
                starport: planetData.starport,
                size: planetData.size,
                atmosphere: planetData.atmosphere,
                hydrographics: planetData.hydrographics,
            ...
            await client_1.db.planet.upsert({
                where: {
                    id: planetId
                },
                update: {
...
async function seedTestCampaign() {
    ...
    // Create campaign
    const campaign = await client_1.db.campaign.upsert({
    ...
    const members = [
        { userId: 'test_gamemaster_1', role: 'GAMEMASTER' },
        { userId: 'test_player_1', role: 'PLAYER' },
        { userId: 'test_player_2', role: 'PLAYER' }
    ];
    for (const memberData of members) {
        await client_1.db.campaignMember.upsert({
            where: {
                campaignId_userId: {
    ...
    for (const characterData of characters) {
        ...
        // Create character
        const character = await client_1.db.character.upsert({
            where: { id: characterData.id },
            update: charInfo,
            create: charInfo
        });
        // Create characteristics
        await client_1.db.characteristics.upsert({
        ...
        for (const skill of skills) {
            await client_1.db.characterSkill.upsert({
                where: {
        ...
        for (const item of equipment) {
            await client_1.db.characterEquipment.create({
                data: {
                    ...item,
                    characterId: character.id
                }
...
async function clearDatabase() {
    ...
    await client_1.db.characterEquipment.deleteMany();
    await client_1.db.characterSkill.deleteMany();
    await client_1.db.lifeEvent.deleteMany();
    await client_1.db.characteristics.deleteMany();
    await client_1.db.combatAction.deleteMany();
    await client_1.db.combatSession.deleteMany();
    await client_1.db.sessionNote.deleteMany();
    await client_1.db.handout.deleteMany();
    await client_1.db.houseRule.deleteMany();
    await client_1.db.customContent.deleteMany();
    await client_1.db.diceRoll.deleteMany();
    await client_1.db.encounter.deleteMany();
    await client_1.db.session.deleteMany();
    await client_1.db.character.deleteMany();
    await client_1.db.tradeRoute.deleteMany();
    await client_1.db.tradeGood.deleteMany();
    await client_1.db.planet.deleteMany();
    await client_1.db.starSystem.deleteMany();
    await client_1.db.starship.deleteMany();
    await client_1.db.campaignMember.deleteMany();
    await client_1.db.campaign.deleteMany();
