...
export class TestDataManager {
  ...
  async createTestUser(overrides = {}) {
    ...
    try {
      // Register user
      const registrationResponse = await axios.post(`${this.apiURL}/auth/register`, userData);
  ...
  async createTestCharacter(userId, overrides = {}) {
    ...
    try {
      ...
      console.error('Failed to create test character:', error.response?.data || error.message);
      throw error;
    }
  }
  ...
  async createBatchUsers(count = 10) {
    ...
    for (let i = 0; i < count; i += batchSize) {
      ...
      for (let j = 0; j < remaining; j++) {
        batch.push(this.createTestUser({ 
  ...
  async cleanupCharacters() {
    console.log(`Cleaning up ${this.createdData.characters.length} test characters...`);
    
    const cleanupPromises = this.createdData.characters.map(async (character) => {
      try {
        await axios.delete(`${this.apiURL}/characters/${character.id}`, {
          headers: this.getHeaders()
        });
      } catch (error) {
        console.warn(`Failed to delete character ${character.id}:`, error.message);
      }
    });
  ...
  async cleanupCampaigns() {
    console.log(`Cleaning up ${this.createdData.campaigns.length} test campaigns...`);
    ...
    const cleanupPromises = this.createdData.campaigns.map(async (campaign) => {
      try {
        ...
        console.warn(`Failed to delete campaign ${campaign.id}:`, error.message);
      }
    });
  ...
  async validateCharacterData(characterId) {
    try {
      ...
      const validations = {
        ...
        characteristicsInRange: character.characteristics.every(c => c.value >= 1 && c.value <= 18),
      ...
      console.error(`Failed to validate character ${characterId}:`, error.message);
  ...
  async generateLoadTestData(options = {}) {
    ...
    console.log(`🚀 Generating load test data: ${userCount} users, ${charactersPerUser} chars/user, ${campaignCount} campaigns`);
    ...
    try {
      ...
      for (const user of users) {
        ...
        const characters = await this.createBatchCharacters(user.id, charactersPerUser);
