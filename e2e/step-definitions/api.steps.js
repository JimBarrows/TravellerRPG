...
Given('the GraphQL API is running', async function () {
  const response = await this.page.request.post(`${this.config.apiURL}/graphql`, {
    data: {
      query: `
        query HealthCheck {
          __schema {
...
Then('all relationships should be properly resolved', async function () {
  ...
  const expectedTypes = ['STRENGTH', 'DEXTERITY', 'ENDURANCE', 'INTELLIGENCE', 'EDUCATION', 'SOCIAL_STANDING'];
...
Then('the changes should be reflected in subsequent queries', async function () {
  const token = this.getTestData('authToken');
  const character = this.getTestData('existingCharacter');
  const expectedName = this.getTestData('updatedName');
...
Given('I have multiple characters in the system', async function () {
  ...
  for (let i = 0; i < 5; i++) {
    const characterData = this.generateTestCharacter({ name: `Test Character ${i + 1}` });
...
Then('I should be able to navigate pages using cursors', async function () {
  ...
  if (endCursor && firstResponseData.data.characters.pageInfo.hasNextPage) {
    ...
    const secondResponse = await this.page.request.post(`${this.config.apiURL}/graphql`, {
      data: {
        query,
        variables: {
          first: 2,
    ...
    const firstIds = firstResponseData.data.characters.edges.map(e => e.node.id);
    const secondIds = secondResponseData.data.characters.edges.map(e => e.node.id);
