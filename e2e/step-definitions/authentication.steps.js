...
When(
  "I fill in the registration form with valid details on mobile",
  async function () {
    const testUser = this.generateTestUser();
    this.storeTestData("currentUser", testUser);
    ...
    await this.page.evaluate(() =>
      document.querySelector('[data-testid="email-input"]')?.scrollIntoView(),
    );
    await this.fillField('[data-testid="email-input"]', testUser.email);

    await this.page.evaluate(() =>
      document
        .querySelector('[data-testid="password-input"]')
        ?.scrollIntoView(),
    );
    await this.fillField('[data-testid="password-input"]', testUser.password);
    ...
    await this.page.evaluate(() =>
      document
        .querySelector('[data-testid="confirm-password-input"]')
        ?.scrollIntoView(),
    );
    await this.fillField(
      '[data-testid="confirm-password-input"]',
    ...
    await this.page.evaluate(() =>
      document
        .querySelector('[data-testid="first-name-input"]')
        ?.scrollIntoView(),
    );
    await this.fillField(
    ...
    await this.page.evaluate(() =>
      document
        .querySelector('[data-testid="last-name-input"]')
        ?.scrollIntoView(),
    );
    await this.fillField('[data-testid="last-name-input"]', testUser.lastName);
    ...
    await this.page.evaluate(() =>
      document
        .querySelector('[data-testid="username-input"]')
        ?.scrollIntoView(),
    );
    await this.fillField('[data-testid="username-input"]', testUser.username);
  },
...
When("I submit the registration form on mobile", async function () {
  ...
  await this.page.evaluate(() =>
    document.querySelector('[data-testid="register-button"]')?.scrollIntoView(),
  );
...
Then("I should see a verification message", async function () {
  ...
  const message = await this.page.textContent(
...
Then("I should see a verification message on mobile", async function () {
  ...
  const messageElement = await this.page.locator(
    '[data-testid="verification-message"]',
  );
...
Then(
  "I should be redirected to the verification page on mobile",
  async function () {
    await this.page.waitForURL("**/verify-email**");
    await this.waitForElement('[data-testid="verification-page"]');
...
Then(
  "the registration should complete within {int} seconds",
...
Then(
  "API calls should complete within {int} seconds",
  async function (maxSeconds) {
    const response = this.getTestData("registrationResponse");
