...
Given("I have a registered and verified user account", async function () {
  ...
  const verificationResponse = await this.page.request.post(
    `${this.config.apiURL}/auth/verify`,
    {
      data: {
...
When("I enter valid credentials on mobile", async function () {
  ...
  await this.page.evaluate(() =>
    document.querySelector('[data-testid="login-email"]')?.scrollIntoView(),
  );
  await this.fillField('[data-testid="login-email"]', testUser.email);

  await this.page.evaluate(() =>
    document.querySelector('[data-testid="login-password"]')?.scrollIntoView(),
  );
  await this.fillField('[data-testid="login-password"]', testUser.password);
});
...
When("I submit the login form on mobile", async function () {
  ...
  await this.page.evaluate(() =>
    document.querySelector('[data-testid="login-button"]')?.scrollIntoView(),
  );
...
Then("I should be logged in successfully on mobile", async function () {
  ...
  await this.waitForElement('[data-testid="mobile-user-menu"]', {
    timeout: 10000,
  });
});
...
Then("the token should be valid", async function () {
  ...
  const protectedResponse = await this.page.request.get(
...
When("I make authenticated API requests", async function () {
  ...
  const requests = ["/user/profile", "/characters", "/campaigns"];
...
When("the token expires", async function () {
  ...
  const expiredToken =
    "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiYWRtaW4iOnRydWUsImV4cCI6MTYwOTQ2MDgwMH0.invalid";
