...
Given('I am logged in with the same account on all platforms', async function () {
...
Then('the character should appear in my character list', async function () {
  ...
  const character = this.getTestData('crossPlatformCharacter');
  const characterElement = await this.waitForElement(`[data-testid="character-${character.id}"]`);
  
  expect(characterElement).toBeTruthy();
  
  // Verify character name is displayed
  const characterName = await this.page.textContent(`[data-testid="character-${character.id}"] [data-testid="character-name"]`);
...
Then('all character details should be identical', async function () {
  ...
  const displayedName = await this.page.textContent('[data-testid="character-name-display"]');
  expect(displayedName).toBe(originalCharacter.name);
  
  const displayedAge = await this.page.textContent('[data-testid="character-age-display"]');
  expect(displayedAge).toContain(originalCharacter.age.toString());
  
  // Verify characteristics are present
  const characteristics = await this.page.locator('[data-testid^="characteristic-"]');
...
When('I edit the character on mobile', async function () {
  ...
  await this.page.evaluate(() => document.querySelector('[data-testid="edit-character-button"]')?.scrollIntoView());
  ...
  await this.page.evaluate(() => document.querySelector('[data-testid="edit-character-name"]')?.scrollIntoView());
  await this.fillField('[data-testid="edit-character-name"]', updatedName);
  
  // Save changes
  await this.page.evaluate(() => document.querySelector('[data-testid="save-character-button"]')?.scrollIntoView());
...
Then('the changes should sync to desktop within {int} seconds', async function (maxSeconds) {
  ...
  while (!syncCompleted && (Date.now() - startTime) < (maxSeconds * 1000)) {
    ...
    try {
      const characterName = await this.page.textContent(`[data-testid="character-${updatedCharacter.id}"] [data-testid="character-name"]`, { timeout: 2000 });
...
Given('I join a campaign on desktop', async function () {
  ...
  const createCampaignResponse = await this.page.request.post(`${this.config.apiURL}/campaigns`, {
    data: campaignData,
    headers: {
...
When('I access the same campaign on mobile', async function () {
  ...
  await this.page.evaluate((campaignId) => {
    document.querySelector(`[data-testid="campaign-${campaignId}"]`)?.scrollIntoView();
  }, campaign.id);
...
Then('all campaign information should be synchronized', async function () {
  ...
  const campaignName = await this.page.textContent('[data-testid="campaign-name"]');
  expect(campaignName).toBe(campaign.name);
  
  const campaignDescription = await this.page.textContent('[data-testid="campaign-description"]');
...
Then('my participation status should be consistent', async function () {
  ...
  const testUser = this.getTestData('crossPlatformUser');
  const participantElement = await this.waitForElement(`[data-testid="participant-${testUser.username}"]`);
  
  expect(participantElement).toBeTruthy();
  
  // Verify I can access campaign features
  const joinButtonVisible = await this.page.locator('[data-testid="join-campaign-button"]').isVisible().catch(() => false);
  expect(joinButtonVisible).toBe(false); // Should not be visible since already joined
  
  const leaveButtonVisible = await this.page.locator('[data-testid="leave-campaign-button"]').isVisible().catch(() => false);
...
When('another player makes changes that affect me', async function () {
  ...
  const messageResponse = await this.page.request.post(`${this.config.apiURL}/campaigns/${campaign.id}/messages`, {
    data: {
...
Then('the state should be consistent across devices', async function () {
  ...
  const participantElements = await this.page.locator('[data-testid^="participant-"]');
