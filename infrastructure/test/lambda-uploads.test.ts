...
describe('Uploads Lambda Handler', () => {
  ...
  beforeEach(() => {
    ...
    process.env.ALLOWED_ORIGINS = 'http://localhost:5173,https://traveller-rpg.com';
  ...
  describe('CORS Handling', () => {
    test('should handle OPTIONS preflight request', async () => {
      ...
      expect(result.headers).toEqual({
        'Access-Control-Allow-Origin': 'http://localhost:5173,https://traveller-rpg.com',
  ...
  describe('File Key Generation', () => {
    test('should generate user upload key when no characterId provided', async () => {
      ...
      expect(mockGetSignedUrl).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          Key: expect.stringMatching(/^users\/user-123\/uploads\/\d+-test-uuid-123\.jpg$/),
        }),
        expect.any(Object)
      );
    });
    ...
    test('should generate character portrait key when characterId provided', async () => {
      ...
      expect(mockGetSignedUrl).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({
          Key: expect.stringMatching(/^characters\/char-456\/portraits\/\d+-test-uuid-123\.png$/),
        }),
        expect.any(Object)
      );
    });
  ...
  describe('S3 Integration', () => {
    test('should create PUT and GET presigned URLs', async () => {
      ...
      const putCall = mockGetSignedUrl.mock.calls.find(call => 
        call[1] instanceof PutObjectCommand
      );
      const getCall = mockGetSignedUrl.mock.calls.find(call => 
    ...
    test('should return presigned URLs and metadata', async () => {
      ...
      expect(responseBody).toHaveProperty('uploadUrl', 'https://presigned.url/test');
      expect(responseBody).toHaveProperty('viewUrl', 'https://presigned.url/test');
