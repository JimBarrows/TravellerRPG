...
describe("Environment Configuration Functions", () => {
  ...
  describe("validateEnvironmentConfig", () => {
    ...
    test("should throw error for missing app name", () => {
      ...
      expect(() => validateEnvironmentConfig(invalidConfig)).toThrow(
    ...
    test("should throw error for invalid lambda memory size", () => {
      ...
      expect(() => validateEnvironmentConfig(invalidConfig)).toThrow(
        "Lambda memory size must be between 128 and 10240 MB",
      );
    });
    ...
    test("should throw error for invalid max concurrent users", () => {
      ...
      expect(() => validateEnvironmentConfig(invalidConfig)).toThrow(
        "Max concurrent users must be at least 1",
      );
    });
  });
...
describe("Configuration Consistency", () => {
  ...
  test("prod should have stricter security than dev", () => {
    const prodSecurityFeatures = Object.values(prodConfig.security).filter(
      Boolean,
    ).length;
    const devSecurityFeatures = Object.values(devConfig.security).filter(
      Boolean,
    ).length;
  ...
  test("prod should have higher performance limits than dev", () => {
    expect(prodConfig.performance.maxConcurrentUsers).toBeGreaterThan(
      devConfig.performance.maxConcurrentUsers,
    );
    expect(prodConfig.lambda.memorySize).toBeGreaterThanOrEqual(
      devConfig.lambda.memorySize,
    );
  });
});
